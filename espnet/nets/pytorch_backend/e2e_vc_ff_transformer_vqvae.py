...
from espnet.nets.pytorch_backend.vqvae.quantizer import VectorQuantizer
from espnet.nets.pytorch_backend.vqvae.jitter import Jitter
...
class FeedForwardTransformerLoss(torch.nn.Module):
    ...
    def forward(self, after_outs, before_outs, ys, ilens, olens):
...
class FeedForwardTransformerVQVAE(TTSInterface, torch.nn.Module):
    ...
    def add_arguments(parser):
        ...
        group.add_argument(
            ...
            type=int,
            default=512,
            help="Quantizer's codebook number",
        )
        group.add_argument(
            "--quantizer-embed-dim",
            type=int,
            default=64,
            help="Quantizer's codebook dimension",
        )
        group.add_argument(
            "--quantizer-embed-normalize",
            type=strtobool,
            default=True,
            help="Normalize quantizer's codebook or not",
        )
        group.add_argument(
            "--quantizer-beta",
            type=float,
            default=0.01,
            help="Quantizer's loss weight of Z_e",
        )
        group.add_argument(
            "--quantizer-jitter-rate",
            type=float,
            default=0.12,
            help="jitter rate for quantized vectors",
        )
        # training related
        group.add_argument(
            "--transformer-init",
    ...
    def __init__(self, idim, odim, args=None):
        ...
        Args:
            ...
            args (Namespace, optional):
                ...
                - quantizer-embed-num: Quantizer's codebook number
                - quantizer-embed-dim: Quantizer's codebook dimension
                - quantizer-embed-normalize: Normalize quantizer's codebook or not
                - quantizer-beta: Quantizer's loss weight of Z_e
                - quantizer-jitter-rate: jitter rate for quantized vectors
        ...
        self.idim = idim
        self.odim = odim
        self.zdim = zdim = args.quantizer_embed_dim
        ...
        if args.eprenet_conv_layers != 0:
            ...
            encoder_input_layer = torch.nn.Sequential(
                EncoderPrenet(
                    idim=idim * self.reduction_factor,
                    elayers=0,
            ...
            encoder_input_layer = torch.nn.Linear(
                idim * args.reduction_factor, args.adim
            )
        else:
        ...
        zdim_extend = zdim * args.reduction_factor
        self.lat_z = torch.nn.Linear(args.adim, zdim_extend)

        # define quantizer
        self.quantizer = VectorQuantizer(
            z_num=args.quantizer_embed_num,
            z_dim=zdim,
            normalize=args.quantizer_embed_normalize,
            beta=args.quantizer_beta,
        )
        self.jitter = Jitter(probability=args.quantizer_jitter_rate)

        # define additional projection for speaker embedding
        if self.spk_embed_dim is not None:
            if self.spk_embed_integration_type == "add":
                self.projection = torch.nn.Linear(
                    self.spk_embed_dim, zdim_extend
                )
            else:
                self.projection = torch.nn.Linear(
                    zdim_extend + self.spk_embed_dim, zdim_extend
                )
    ...
    def _forward(
        ...
        if self.reduction_factor > 1:
            if Lmax % self.reduction_factor != 0:
                xs = xs[:, : -(Lmax % self.reduction_factor), :]
                Lmax -= Lmax % self.reduction_factor
            xs_in = xs.contiguous().view(
                B,
                int(Lmax / self.reduction_factor),
                idim * self.reduction_factor,
            )
            ilens_in = ilens.new(
                [ilen // self.reduction_factor for ilen in ilens]
            )
        else:
            xs_in, ilens_in = xs, ilens

        # forward encoder
        x_masks = self._source_mask(ilens_in)
        hs, _ = self.encoder(xs_in, x_masks)  # (B, Lmax//reduction_factor, adim)

        # vector quantize
        hs = self.lat_z(hs).view(B, Lmax, -1)
        if is_inference:
            hid = self.quantizer.encode(hs)
            hs = self.quantizer.decode(hid)
        else:
            hs, vq_loss, vq_detail = self.quantizer(hs)
            hs = self.jitter(hs)
        hs = hs.contiguous().view(B, -1, self.zdim * self.reduction_factor)
        ...
        if olens is not None:
            ...
            h_masks = None
        zs, _ = self.decoder(hs, h_masks)  # (B, Lmax//reduction_factor, adim)
        ...
        if is_inference:
            ...
            return before_outs, after_outs, vq_loss, vq_detail 

    def forward(self, xs, ilens, ys, olens, spembs=None, extras=None, *args, **kwargs):
        """Calculate forward propagation.

        Args:
            xs (Tensor): Batch of padded character ids (B, Lmax, idim).
        ...
        # forward propagation
        before_outs, after_outs, vq_loss, vq_detail = self._forward(
            xs, ilens, ys, olens, spembs=spembs, is_inference=False
        )
        ...
        if self.postnet is None:
            l1_loss, l2_loss = self.criterion(
                None, before_outs, ys, ilens, olens
            )
        else:
            l1_loss, l2_loss = self.criterion(
                after_outs, before_outs, ys, ilens, olens
            )
        if self.loss_type == "L1":
            loss = l1_loss + vq_loss
        elif self.loss_type == "L2":
            loss = l2_loss + vq_loss
        elif self.loss_type == "L1+L2":
            loss = l1_loss + l2_loss + vq_loss
        else:
    ...
    def base_plot_keys(self):
        ...
        """
        plot_keys = ["loss", "l1_loss", "l2_loss", "vq_loss", "entropy"]
        if self.use_scaled_pos_enc:

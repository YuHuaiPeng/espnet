...
def calculate(file_list, gt_file_list, args, MCD):
    ...
    for i, cvt_path in enumerate(file_list):
        cvt_path_basename = get_basename(cvt_path)
        processed_cvt_path = f"{((int(cvt_path_basename[-4:-2])-1)*10+int(cvt_path_basename[-2:])):03}"
        print(processed_cvt_path)
        corresponding_list = list(
            filter(lambda gt_path: processed_cvt_path in get_basename(gt_path), gt_file_list)
        )

...
minf0=$(grep ${trgspk} conf/all.f0 | awk '{print $2}')
maxf0=$(grep ${trgspk} conf/all.f0 | awk '{print $3}')
${decode_cmd} ${mcd_file} \
    local/mcd_calculate.py \
        --wavdir ${wavdir} \
        --gtwavdir ${db_root}/tmsv/${trgspk} \
        --mcep_dim ${mcep_dim} \
        --shiftms ${shift_ms} \
        --f0min ${minf0} \
        --f0max ${maxf0}
...
local/ob_eval/traditional2simplified.py data/${trgspk}_${name}/text data/${trgspk}_${name}/utt2spk > ${asr_data_dir}/${set_name}/text

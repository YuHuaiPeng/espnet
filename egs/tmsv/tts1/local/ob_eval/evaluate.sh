...
# Generate original text for the dev/test set
text=${asr_data_dir}/${set_name}/text
[ -e ${text} ] && rm ${text}
for spk_dir in $(find -L ${db_root}/TMSV/ -mindepth 1 -maxdepth 1 -type d -iname "SP*" | sort); do
    spk_id=$(basename $spk_dir)
    local/make_transcription_per_spk.py -s $spk_id \
        -f $spk_dir/${spk_id}_read.txt -t original >> ${text}.tmp || exit 1
done
local/ob_eval/traditional2simplified.py ${text}.tmp data/${set_name}/utt2spk > ${asr_data_dir}/${set_name}/text
...
local/ob_eval/score_sclite_wo_dict_modified.sh ${asr_result_dir}.${api}/${set_name}

...
def get_chewing_tone(syllable):
    if re.match(".+ˊ", syllable):
        tone = 2
    elif re.match(".+ˇ", syllable):
        tone = 3
    elif re.match(".+ˋ", syllable):
        tone = 4
    elif re.match(".+˙", syllable):
        tone = 5
    else:
        tone = 1
    return tone
        ...
        if self.target == "IPA":
            ...
            with open_workbook(self.forpa_excel) as wb:
                ...
                for i in range(3, nrows):
                    ...
                    source_syllable, target_syllable = (
                        row[pinyin2col[self.source]],
                        row[pinyin2col[self.target]],
                    )
                    pinyin_map[source_syllable] = target_syllable
        return pinyin_map

    def transform(self, syllable):
        if syllable in self.map:
            return self.map[syllable].strip()
        else:
            # print("=>", syllable)

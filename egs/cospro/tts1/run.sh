...
verbose=10    # verbose option (if set > 1, get more log)
...
# config files
train_config=conf/cospro_tts_nosped.yaml
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    local/data_prep.sh ${datadir} data/${org_set}
fi
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    # Extract x-vector
    for name in ${train_set} ${dev_set} ${eval_set}; do
        sid/nnet3/xvector/extract_xvectors.sh --cmd "$train_cmd --mem 4G" --nj 2 \
...
if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
    ...
    for name in ${dev_set} ${eval_set}; do
        mcd_file=${outdir}_denorm/${name}/mcd.log
        ${decode_cmd} ${mcd_file} \
            mcd_calculate.py \
                --wavdir ${outdir}_denorm/${name}/wav \
                --gtwavdir ${datadir}/wav \
                --mcep_dim 24 \
                --shiftms 5 \
                --f0min 40 \
                --f0max 400
...
if [ ${stage} -le 8 ] && [ ${stop_stage} -ge 8 ]; then
    echo "stage 8: Objective evaluation: CER/WER"
